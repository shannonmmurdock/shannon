...
Trigger DocusignStatusTrigger_AT  on dsfs__DocuSign_Status__c (before update, after update) {
    ...
    if(Trigger.isBefore && Trigger.isUpdate && RecursiveTriggerHandler.blockBeforeUpdate == false) {
        ...
        if(leadIdSet.size() > 0)
        {
            for(Lead dbLead : [SELECT Id,isSign__c,Status,RFI_Form_Info_Hidden__c, Auto_Qualified__c, Provider_Signature__c, DV_Recipient__c from Lead WHERE Id in:leadIdSet AND Status = :'Referred' AND Sub_Status__c = 'Pending Diagnosis Verification']){
                ...
                dbLead.Provider_Signature__c = dbLead.DV_Recipient__c;
                system.debug(dbLead.DV_Recipient__c);
                leadList.add(dbLead);
            }
            if(leadList.size() > 0){
        ...
        If(parentWishIdSet.Size() > 0){
            for(Case planningCase : [SELECT Id,Date_Received_for_Child_s_Medical_Summar__c,Child_s_Medical_Summary_Form__c FROM Case WHERE ParentId IN : parentWishIdSet AND RecordTypeId =: wishPlanningRecordTypeId ]){
                planningCase.Date_Received_for_Child_s_Medical_Summar__c  = System.today();
                planningCase.Child_s_Medical_Summary_Form__c = True;
        ...
        if(dstsIds.size() > 0)
        {
            dstsStatusRecMap.putAll([SELECT id, Docusign_Hidden_Contact__c,Docusign_Hidden_Contact__r.Account.Volunteer_Manager__c,
                ...
                if(dsts.Docusign_Hidden_Contact__c  != Null)
                {
                    ...
                    if(dstsStatusRecMap.size() > 0)
                    {
                        if(dstsStatusRecMap.get(dsts.id).Docusign_Hidden_Contact__r.Account.Volunteer_Manager__c != Null)
                            approvalreq.setNextApproverIds(new Id[]{dstsStatusRecMap.get(dsts.id).Docusign_Hidden_Contact__r.Account.Volunteer_Manager__c});
                        else 
                            approvalreq.setNextApproverIds(new Id[]{dstsStatusRecMap.get(dsts.id).Docusign_Hidden_Contact__r.OwnerId});
                    }
